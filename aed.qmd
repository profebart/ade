...
Associamos às classes acima a denominação de **variáveis** quando estamos interessados em atuar sobre elas em projetos, em ações de alteração de níveis de resposta, seja na rotina do trabalho, na pesquisa, enfim, em busca de otimizações de desempenho. Mais especificamente podemos denominá-las como sendo **variáveis-resposta** ou variáveis **dependentes** (Y). Sendo estas variáveis que possa controlar ou agir, verificar e alterar as condições do processo, denominamos estas variáveis como sendo **variáveis de controle** (afetam ou determinam uma característica final do produto) ou **de verificação** (atuam na transformação do produto no processo e futuramente entregarão a(s) especificação(ões) desejadas pelo cliente).
...
Fávaro e Belfiore propuseram uma relação intressante entre o tipo de variável e as possíveis representações na estatística univariada, como mostra a figura:

![](images/tipo_var_estat_desc_univar_favero.png){fig-align="center"}
...
![](images/z_2_sigmas.png){fig-align="center"}
...
```
Assim, pode-se obter o valor da variável X (quantil) a partir do Z Score com o comando **qnorm** , da seguinte forma,:
...
O valor Z=2 pode ser obtido com a entrada de dados agora para média e desvio-padrão padronizados:
...
Se preferir, use o vetor criado anteriormente:
```{r}
...
Vamos analisar o caso em que se tenha velocidades compreendidas entre valroes máximos e mínimos desejados (áreas intermediárias).
